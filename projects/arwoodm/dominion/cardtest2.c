...
int main() {
	...
	*********************************************/
	...
	/*********************************************
	//variables before adventurer played
	*********************************************/
	int currPlayer = whoseTurn(&G);
	int cardsInHand = G.handCount[currPlayer];
	int deckCount = G.deckCount[currPlayer];
	int initialTreasure = G.supplyCount[4] + G.supplyCount[5] + G.supplyCount[6];
	...
	/********************************************
	//variables after adventurer played
	********************************************/

	//hand count increase
	if (G.handCount[currPlayer] == cardsInHand + 2) {
		printf("PASSED: current hand count increased by 2\n\n");
	} 
	else {
	...
	if (initialTreasure > newTreasure) {
		printf("PASSED: current hand treasure increased \n\n");
	}
	else {
	...
	//deck count decrease
	if (G.deckCount[currPlayer] < deckCount) {
