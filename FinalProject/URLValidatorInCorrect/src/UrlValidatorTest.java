...
public class UrlValidatorTest extends TestCase {
   ...
   public void testIsValid(ResultPair[][] testObjects, long options)
   {
	  ...
	  //O(n^4)...should i write it differently??
	  //scheme
		  ...
		  for (int j = 0; j < testObjects[1].length; j++) {
			  ...
			  for (int k = 0; k < testObjects[2].length; k++) {
				  ...
				  for (int m = 0; m < testObjects[3].length; m++) {
					  validity = testObjects[0][i].valid && testObjects[1][j].valid && testObjects[2][k].valid && testObjects[3][m].valid;
					  url = testObjects[0][i].item + testObjects[1][j].item + testObjects[2][k].item + testObjects[3][m].item;
					  result = urlVal.isValid(url);
					  if (validity != result) {
   ...
   public static void main(String[] args) {
	   ...
	   ResultPair[][] testObjects = {scheme, authority, port, path};
